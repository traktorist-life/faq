...
private val exitDocumentFromRussia = FaqItem(
    question = "Основание выезда из РФ",
    answer = listOf(
        "Для выезда из РФ по сухопутной границе требуется основание (см. вопрос про проезд на автомобиле).".paragraph(),
        "Для выезда самолётом или морскими путями разрешение не требуется.".paragraph(),
    ),
    listOf(Tag.Transfer),
    null
)
...
private val nonVisaAvia = FaqItem(
    ...
    answer = "Наиболее популярные безвизовые рейсы это Москва-Белград-Подгорица (AirSerbia) и Москва-Стамбул-Подгорица (TurkishAirlines)",
    Tag.Transfer,
)
...
private val touristRegister = FaqItem(
    ...
    answer = listOf(
        ...
        "Если к вам в вашу недвижимость приехали родственники, их тоже надо регистрировать. При этом если у родственников фамилия совпадает с вашей, то регистрация бесплатна. Если фамилия другая, то регистрация уже на общих основаниях, см. ниже.".paragraph(),
        ...
        "Для регистрации нужны паспорта всех прибывших, хозяин жилья, где вы поселились и его доказательство владения этим жильем. +Деньги на тур.налог.".paragraph(),
...
private val sputnikAcceptance = FaqItem(
    ...
    answer = listOf(
        "С 11 марта для въезда никакие документы про ковид не нужны. Такой режим установлен и продляется каждые 2 недели ввиду благоприятной эпидемиологической обстановки. Также с 8 апреля в помещениях (кроме медицинских) можно не носить маски.".paragraph(),
        ...
        "По отзывам, в аэропортах ещё не все знают об изменениях. Сделать ПЦР/антиген или доказать работникам при регистрации вашу правоту (у них есть правила проезда в компьютере, мало кто читает) - здесь каждый решает для себя.".paragraph(),
    ),
    listOf(Tag.Transfer),
    null
)
...
private val belgradeTransferOneHour = FaqItem(
    ...
    answer = listOf(
        ...
        "Главное берите такую стыковку одним билетом, чтобы не получать багаж и не проходить регистрацию в Белграде.".paragraph(),
        "Сообщения о минировании".header(),
        "После 12 марта много рейсов AirSerbia из Белграда в Москву после вылета было возвращено обратно в Белград из-за ложных сообщений о бомбах на борту. Это приводит к опозданию самолётов относительно расписания и к проблемам в стыковках. Пока злоумышленника не нашли, стоит особенно осторожно относиться к таким коротким стыковкам и иметь запасной план на случай возникновения проблемы в стыковке в Сербии.".paragraph(),
    ),
    listOf(Tag.Transfer),
    null
)
private val carTransfer = FaqItem(
    ...
    answer = listOf(
        "Чтобы проехать нужна шенгенская виза и основание выезда из РФ.".paragraph(),
        "Основание выезда".header(),
        "Написанное далее НЕ актуально, если вы выезжаете в европу через границу Белоруссии.".paragraph(),
        "В отличие от авиаперелёта, пересечение наземной границы РФ требует основания выезда, а туризма списке разрешённых оснований нет.".paragraph(),
        "Полный список тех, кому разрешено пересекать сухопутную границу, указан в распоряжении правительства 763-р (см http://www.consultant.ru/document/cons_doc_LAW_348748/).".paragraph(),
        "Если вы не дипломат и не на службе, то самые частые виды оснований это наличие ВНЖ, недвижимости, родственников, лечение.".paragraph(),
        "Любой вид основания должен иметь документарное подтверждение. Если документ не на русском языке (например, лист непокретности), то он должен быть переведён и нотариально заверен.".paragraph(),
        "Если основание есть у одного из супругов, второго тоже пропускают, но нужен документ о браке.".paragraph(),
    ),
    listOf(Tag.Transfer),
    null
)
private val trainTransfer = FaqItem(
    ...
    answer = listOf(
        "Всё аналогично автомобилю. Исключение одно - при путешествии поездом в Финляндию разрешение на выезд из РФ не требуется.".paragraph(),
    ),
    listOf(Tag.Transfer),
    null
)
...
private val airSerbiaTicketTransfer = FaqItem(
    question = "Замена пассажира в билете/переоформление билета AirSerbia",
    answer = listOf(
        "AirSerbia не позволяет поменять ФИО в билете, как и не позволяет заменить пассажира.".paragraph(),
